...
exports.register = function(db, endpoints) {
  var event = require("./event").getter(db),
      ...
      chartData = require("./chart_data").getter(db);
  ...
  endpoints.ws.push(
    ...
    endpoint("/1.0/chart_data/get", chartData)
  );
  ...
  endpoints.http.push(
    ...
    endpoint("GET", "/1.0/chart_data", chartDataGet),
    endpoint("GET", "/1.0/chart_data/get", chartDataGet)
  );
  ...
  function chartDataGet(request, response) {
    request = url.parse(request.url, true).query;
    ...
    // If the limit is not specified, or too big, use the maximum limit.
    if (!("step" in request)) request.step = step = 1e4;
    request.stop = Math.floor(Date.now() / request.step) * request.step;
    if (!("start" in request)) request.start = 0;
    if (!(limit <= 100)) limit = 100;
    if (!("init_value" in request)) request.init_value = 0;

    var start = new Date(request.start), stop = new Date(request.stop);
    if ((stop - start) / step > limit) request.start = new Date(stop - step * limit);

    chartData(request, function(data) {
      response.writeHead(200, headers);
      response.end(JSON.stringify(data));
    });
  }
