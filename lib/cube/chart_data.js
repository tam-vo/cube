      ...
      callbackTimeoutId = setTimeout(function() {
        ...
        for (time in data) {
          ...
          if (emptyData) {
            ...
            if (isNOE(hashTimeFrame["high"])) hashTimeFrame["high"] = Math.max(hashTimeFrame["open"], hashTimeFrame["close"]);
            if (isNOE(hashTimeFrame["low"])) hashTimeFrame["low"] = Math.min(hashTimeFrame["open"], hashTimeFrame["close"]);
          }
          lastTime = time;
        }
        var lastValue = null;
